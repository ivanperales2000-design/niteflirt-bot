...
class BotDefinitivo {
  constructor() {
    ...
    this.lastCheck = new Date();
          this.checkInterval = 30 * 60 * 1000; // 30 minutos para verificar mensajes nuevos
    this.restartAttempts = 0;
    this.maxRestartAttempts = 5;
  }

  async init() {
    console.log('🧠 Iniciando BOT DEFINITIVO de Niteflirt...\n');
    
    try {
      this.browser = await chromium.launch({ 
        headless: true, // Headless para 24/7
        slowMo: 100,
  ...
  async contactInactiveClients() {
    console.log('📞 Contactando clientes inactivos (5-6 días)...');
    ...
    try {
      ...
      for (const selector of unreadSelectors) {
        if (await chatElement.locator(selector).count() > 0) {
          console.log(`✅ Badge de no leído encontrado: ${selector}`);
          return true;
        }
      }
      
      // 2. Verificar el texto del chat para mensajes sin responder
      const chatText = await chatElement.innerText().catch(() => '');
      console.log(`📝 Texto del chat: "${chatText}"`);
      
      // Verificar si el último mensaje no es del bot
      if (chatText && !chatText.includes('You:') && !chatText.includes('You :')) {
        // Verificar si hay palabras clave que indiquen un mensaje reciente
        const recentKeywords = ['bra lines', 'hello', 'hi', 'hey', 'how are you', 'what', 'when', 'where', 'why'];
        const lowerText = chatText.toLowerCase();
        
        for (const keyword of recentKeywords) {
          if (lowerText.includes(keyword)) {
            console.log(`✅ Mensaje reciente detectado con keyword: "${keyword}"`);
            return true;
          }
        }
        
        // Si no hay keywords específicas, verificar si el texto parece un mensaje reciente
        if (chatText.length > 10 && chatText.length < 200) {
          console.log(`✅ Mensaje reciente detectado por longitud: ${chatText.length} caracteres`);
          return true;
        }
      }
      ...
      console.error('❌ Error verificando mensajes no leídos:', error.message);
      return false;
    }
  }
  ...
  async generateContextualResponse(clientMemory, lastMessage) {
    ...
    try {
      ...
      // Fallback responses contextuales
      return this.getContextualFallbackResponse(clientMemory, lastMessage);
  ...
  getContextualFallbackResponse(clientMemory, lastMessage) {
  ...
  async sendMessage(message) {
    ...
    try {
      ...
      // Buscar campo de entrada con múltiples selectores mejorados
      ...
      // Escribir el mensaje con mejor manejo
      ...
      // Buscar botón de enviar con más selectores
      const sendSelectors = [
        'button[type="submit"]',
        '.send-button',
        '.submit-button',
  ...
  async startDefinitiveMonitoring() {
    console.log('🎯 INICIANDO BOT DEFINITIVO 24/7...\n');
    console.log(`⏰ Intervalo de verificación: ${this.checkInterval / (1000 * 60)} minutos`);
      ...
      while (this.isMonitoring) {
        try {
          ...
          if (now - this.lastCheck > this.checkInterval) {
            ...
            // 2. Contactar clientes inactivos (cada 5-6 días)
  ...
  async restartBot() {
    ...
    try {
      ...
      await this.startDefinitiveMonitoring();
...
async function main() {
  ...
  try {
    await bot.startDefinitiveMonitoring();
  } catch (error) {
