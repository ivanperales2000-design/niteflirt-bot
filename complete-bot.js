...
class CompleteBot {
  constructor() {
    ...
    this.lastInactiveCheck = new Date();
    this.inactiveCheckInterval = 30 * 60 * 1000; // 30 minutos
  }

  async init() {
    console.log('🤖 Iniciando Complete Bot de Niteflirt...\n');
  ...
  async generateResponse(clientName = 'Cliente', chatHistory = [], language = 'english', isInactive = false, isNewMessage = false) {
    ...
    try {
      ...
      } else if (isNewMessage && chatHistory.length === 0) {
  ...
  async contactInactiveClients() {
    ...
    try {
      ...
      // Contactar a los primeros 2 clientes inactivos
      ...
      for (const client of clientsToContact) {
        ...
        const contextualMessage = await this.generateResponse(
          client.name, 
          chatHistory, 
          language, 
          true, // isInactive
  ...
  async startCompleteMonitoring() {
    console.log('🎯 Iniciando monitoreo completo automático...\n');
    ...
    try {
      ...
      console.log(`📊 Monitoreo completo iniciado. Mensajes iniciales: ${this.lastMessageCount}`);
      console.log('🔄 Bot en modo monitoreo continuo...');
      console.log('💡 Presiona Ctrl+C para detener el monitoreo\n');
      
      this.isMonitoring = true;
      
      // 7. Bucle de monitoreo completo
      while (this.isMonitoring) {
        try {
          ...
          // Verificar si es momento de contactar clientes inactivos (cada 30 minutos)
          const now = new Date();
...
async function main() {
  ...
  try {
    console.log('🎯 Iniciando bot completo con todas las funcionalidades...');
    await bot.startCompleteMonitoring();
  } catch (error) {
