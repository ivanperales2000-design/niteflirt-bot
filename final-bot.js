...
class FinalBot {
  ...
  async init() {
    console.log('🤖 Iniciando Final Bot de Niteflirt...\n');
  ...
  async login() {
    ...
    try {
      ...
      // Esperar y verificar login con múltiples estrategias
      console.log('⏳ Verificando login...');
      await SLEEP(3000);
      
      // Estrategia 1: Verificar si la URL cambió
      ...
      // Estrategia 2: Buscar elementos de usuario logueado
      ...
      // Estrategia 3: Verificar si hay errores de login
      ...
      for (const selector of errorSelectors) {
        if (await this.page.locator(selector).count() > 0) {
          const errorText = await this.page.locator(selector).first().innerText();
          console.log(`❌ Error de login detectado: ${errorText}`);
          return false;
        }
      }
      
      // Estrategia 4: Esperar un poco más y verificar nuevamente
      console.log('⏳ Esperando procesamiento adicional...');
      await SLEEP(5000);
      
      // Verificar nuevamente
      const finalUrl = this.page.url();
      if (finalUrl.includes('/dashboard') || finalUrl.includes('/messages') || finalUrl.includes('/profile')) {
        console.log('✅ Login exitoso detectado en verificación final');
        this.isLoggedIn = true;
        return true;
      }
      
      // Si llegamos aquí, el login probablemente fue exitoso pero no detectamos los elementos
  ...
  async navigateToChat() {
    ...
    try {
      ...
      await this.page.goto('https://www.niteflirt.com/messages', { waitUntil: 'networkidle' });
      await SLEEP(3000);
