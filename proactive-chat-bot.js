...
class ProactiveChatBot {
  ...
  async init() {
    console.log('🤖 Iniciando Proactive Chat Bot de Niteflirt...\n');
  ...
  async generateProactiveMessage(clientName = 'Cliente') {
    console.log('🧠 Generando mensaje proactivo...');
    
    try {
      const proactiveMessages = [
        `¡Hola ${clientName}! 😊 ¿Cómo estás hoy? Me encantaría saber cómo va todo contigo 💕`,
        `Hey ${clientName}! 🌟 ¿Qué tal tu día? Me acordé de ti y quería saludarte 😘`,
        `¡Hola ${clientName}! 💖 ¿Cómo te va? Espero que estés teniendo un día maravilloso ✨`,
        `Hey ${clientName}! 😍 ¿Qué tal todo? Me gustaría charlar un poco contigo 💭`,
        `¡Hola ${clientName}! 🌸 ¿Cómo estás? Me encantaría saber más sobre ti 💫`,
        `Hey ${clientName}! 💕 ¿Qué tal tu día? ¿Hay algo interesante que quieras compartir? 😊`,
        `¡Hola ${clientName}! ✨ ¿Cómo va todo? Me gustaría conectar contigo de nuevo 💖`,
        `Hey ${clientName}! 🌺 ¿Qué tal estás? ¿Te acuerdas de mí? 😘`
      ];
      
      const randomMessage = proactiveMessages[Math.floor(Math.random() * proactiveMessages.length)];
      console.log(`💬 Mensaje proactivo generado: "${randomMessage}"`);
      return randomMessage;
      
    } catch (error) {
      console.error('❌ Error generando mensaje proactivo:', error.message);
      return `¡Hola ${clientName}! 😊 ¿Cómo estás hoy? Me encantaría charlar contigo! 💕`;
    }
  }
  ...
  async contactClient(targetUsername = null) {
    console.log(`🎯 Iniciando contacto proactivo${targetUsername ? ` con ${targetUsername}` : ' con cliente'}...\n`);
    ...
    try {
      ...
      // 6. Generar mensaje proactivo
      const proactiveMessage = await this.generateProactiveMessage(targetUsername || 'Cliente');
...
// Función principal
async function main() {
  const bot = new ProactiveChatBot();
...
module.exports = { ProactiveChatBot };
